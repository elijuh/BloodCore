...
public class LoadCommand extends SubCommand {
    public LoadCommand() {
        super("load", ImmutableList.of(), "blood.admin", "/world load");
    ...
    public void onExecute(CommandSender sender, String[] args) {
        ...
        for (File file : container) {
            if (file.getName().equalsIgnoreCase(worldName)) {
                //Core.i().getServer().createWorld(new WorldCreator(file.getName()).generator(new VoidGenerator()));
                worldExists = true;
                break;
            }
        }
        if (worldExists) {
            ...
            sender.sendMessage(ChatUtil.color(Config.CORE_PREFIX.getString() + "&aLoaded " + worldName));
            return;
        }
        sender.sendMessage(ChatUtil.color(Config.CORE_PREFIX.getString() + "&cCouldn't find " + worldName));
